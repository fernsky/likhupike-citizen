...
import { useUploadCitizenshipBackMutation } from "@/store/services/citizenApi";
import { Badge } from "@/components/ui/badge";
import { ImageViewer } from "@/components/shared/image-viewer";

interface CitizenshipBackUploadCardProps {
  title: string;
  description: string;
...
export function CitizenshipBackUploadCard({
  title,
  description,
  documentInfo,
}: CitizenshipBackUploadCardProps) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setIsUploading(true);
      const response = await uploadCitizenshipBack(formData);
      ...
      console.error("Error uploading citizenship back:", error);
        ...
        <CardContent className="pt-4">
          ...
          <div>
            ...
            <Button
              ...
              className="w-full"
              onClick={() =>
                document.getElementById("upload-citizenship-back")?.click()
              }
              disabled={isUploading || !canReplaceDocument}
            >
              {!canReplaceDocument ? (
