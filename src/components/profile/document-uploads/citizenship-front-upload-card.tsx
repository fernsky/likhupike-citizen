...
import { FileText, Upload, LockIcon, ZoomIn } from "lucide-react";
import { toast } from "sonner";
import { DocumentInfo, DocumentState } from "@/domains/citizen/types";
import { useUploadCitizenshipFrontMutation } from "@/store/services/citizenApi";
import { Badge } from "@/components/ui/badge";
import { ImageViewer } from "@/components/shared/image-viewer";

interface CitizenshipFrontUploadCardProps {
  title: string;
  description: string;
...
export function CitizenshipFrontUploadCard({
  title,
  description,
  documentInfo,
}: CitizenshipFrontUploadCardProps) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setIsUploading(true);
      const response = await uploadCitizenshipFront(formData);
      ...
      console.error("Error uploading citizenship front:", error);
  ...
  const getDocumentActionMessage = (state: DocumentState | undefined) => {
    if (!state || state === "NOT_UPLOADED") {
      return t("actions.uploadDocument");
    }
    ...
    if (
      ...
      return t("actions.replaceDocument");
    }
        ...
        <CardContent className="pt-4">
          ...
          <div>
            ...
            <Button
              ...
              className="w-full"
              onClick={() =>
                document.getElementById("upload-citizenship-front")?.click()
              }
              disabled={isUploading || !canReplaceDocument}
            >
              {!canReplaceDocument ? (
